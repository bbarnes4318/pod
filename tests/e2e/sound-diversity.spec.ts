// Sound diversity creator UI: the bounded diversity-policy config + recent-
// usage summary on the Sound & Branding screen, and the diversity panel on the
// Studio episode Produce surface. DB-only + UI — no LLM/TTS/network. Ordinary
// interactions only: real .click()/.fill()/keyboard — never force:true,
...
test.describe("Sound diversity — creator config UI", () => {
  ...
  test("4/5: an out-of-bounds value is CLAMPED to its bound on save (never takes effect raw)", async ({ page }, ti) => {
    test.skip(!desktopOnly(ti));
    await gotoSound(page);
    await page.getByTestId("div-hardAssetCooldownEpisodes").fill("9999");
    await page.getByTestId("sound-save").click();
    await expect(page.getByTestId("sound-status")).toContainText(/Saved/i, { timeout: 15000 });
    const cfg = await e2eDb().podcastProductionConfig.findUnique({ where: { podcastId: E2E.podcastId }, select: { diversityPolicy: true } });
  ...
  test("2/6: another owner's podcast is not accessible (owner-scoped)", async ({ page }, ti) => {
    ...
    await page.goto(`/studio/shows/${other}/sound`);
  ...
  test("Rollout 2: effective mode is displayed alongside the configured override", async ({ page }, ti) => {
    ...
    for (const mode of ["inherit", "off", "observe", "soft", "enforce"]) {
      await page.getByTestId("div-rolloutModeOverride").selectOption(mode);
    }
  });
  ...
  test("Rollout 13: rollout mode is keyboard-selectable and saves", async ({ page }, ti) => {
    test.skip(!desktopOnly(ti));
    await gotoSound(page);
    const select = page.getByTestId("div-rolloutModeOverride");
    await select.focus();
    await expect(select).toBeFocused();
    await select.selectOption("enforce");
    await page.getByTestId("sound-save").click();
  ...
  test("Render detail: the diversity panel shows engine, mode, source, selections, and sequence", async ({ page }, ti) => {
    ...
    const script = await db.script.create({ data: { episodeId: epId, version: 1, status: "approved", plainText: "hello world", content: { segments: [{ type: "topic", lines: [{ lineIndex: 0, speakerName: "A", text: "hi" }] }] } as object } });
    await db.episodeAudioRender.create({ data: { episodeId: epId, scriptId: script.id, renderVersion: 1, status: "succeeded", renderMode: "initial",
