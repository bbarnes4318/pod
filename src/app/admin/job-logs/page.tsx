...
function extractQueueIdentity(input: unknown): { queueJobId: string | null; targetVersion: number | null } {
  const value = asObject(input);
  return {
    queueJobId: typeof value?.queueJobId === "string" ? value.queueJobId : null,
    targetVersion: typeof value?.targetVersion === "number" ? value.targetVersion : null,
  };
}
...
function shortenedId(id: string): string {
...
export default async function JobLogsPage(props: {
  ...
  const distinctTypes = await db.jobLog.groupBy({ by: ["jobType"] });

  const dbWhere: any = {};
  if (filterJobType) dbWhere.jobType = filterJobType;
  if (filterStatus) dbWhere.status = filterStatus;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
      ...
      <div>
        ...
        <p className="pageDesc">
          Newly submitted work appears immediately. This page refreshes every four seconds so waiting and running jobs cannot be hidden by stale browser state.
        </p>
      </div>
        ...
        <form method="GET" style={{ display: "flex", flexWrap: "wrap", gap: "1rem", alignItems: "flex-end" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", flex: "1 1 240px" }}>
            ...
            <input
              ...
              name="search"
              defaultValue={search}
              placeholder="Episode ID, queue ID, timeout..."
              className="input"
            />
          </div>
            ...
            <table className="table" style={{ borderCollapse: "collapse", width: "100%" }}>
              ...
              <tbody>
                {filteredLogs.map((log) => {
                  ...
                  const { queueJobId, targetVersion } = extractQueueIdentity(log.input);
                  const active = log.status === "submitted" || log.status === "running";
                  const endMs = active ? nowMs : new Date(log.updatedAt).getTime();
                  const elapsedSeconds = Math.max(
                    0,
                    Math.round((endMs - new Date(log.createdAt).getTime()) / 1000)
                  );
                  const elapsedLabel =
                        ...
                        <details style={{ marginTop: "0.25rem" }}>
                          ...
                          <div style={{ marginTop: "0.5rem", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                            <div>
                              ...
                              <pre style={{ backgroundColor: "var(--bg-secondary)", padding: "0.5rem", borderRadius: "4px", overflowX: "auto", fontSize: "0.75rem", color: "var(--text-primary)", border: "1px solid var(--border-color)", margin: "0.25rem 0 0" }}>
