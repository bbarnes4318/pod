...
import ShowForgeWizard, { type WizardHost, type WizardTeam, type WizardInitial } from "@/components/studio/ShowForgeWizard";
import { AppPage, PageHeader } from "@/components/studio";

export const dynamic = "force-dynamic";

export default async function NewStudioShowPage({ searchParams }: { searchParams: Promise<{ topic?: string }> }) {
  ...
  let prefillTopicTitle: string | undefined;
  if (topicId) {
    const topic = await db.topicCandidate.findUnique({ where: { id: topicId }, select: { title: true, summary: true, sport: true, leagueId: true } }).catch(() => null);
    if (topic) {
      const vertical = verticalForTopic(topic.leagueId, topic.sport);
      const text = `${topic.title} ${topic.summary || ""}`.toLowerCase();
      const matchedTeams = teams.filter(team => vertical && team.leagueId === (topic.leagueId || "").toUpperCase()).filter(team => { const nickname = team.name.split(" ").slice(-1)[0].toLowerCase(); return nickname.length >= 4 && text.includes(nickname); }).map(team => team.id);
      initial = { name: topic.title.length > 80 ? `${topic.title.slice(0, 77)}…` : topic.title, verticals: vertical ? [vertical] : [], teams: matchedTeams };
