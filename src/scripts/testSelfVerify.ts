  ...
  return [
    {
      lines: [
        ...
        { lineIndex: 1, speakerName: "Mercer", isFactualClaim: true, text: "And Boone said they flat-out quit.", evidenceRefs: [{ type: "game", id: "f1" }] }, // invented quote
        { lineIndex: 2, speakerName: "Zabala", isFactualClaim: true, text: "They're 48-38, that's real.", evidenceRefs: [{ type: "game", id: "f1" }] }, // grounded
        { lineIndex: 3, speakerName: "Mercer", isFactualClaim: false, text: "Oh, come on." }, // not a claim
      ],
    },
  ];
}
...
async function main() {
  ...
  await check("semantic pass: flags + rewrites a subject-mismatch line (right figure, wrong team)", async () => {
    const segs = [
      { lines: [{ lineIndex: 0, speakerName: "Zabala", isFactualClaim: true, text: "The Yankees are 39-48, nine under.", evidenceRefs: [{ type: "game", id: "f1" }] }] },
    ];
    ...
    const r = await selfVerifyAndCorrect(segs, { evidenceByRefId: evByRef, fullEvidenceText: full, hostNames: ["Zabala", "Mercer"], rewrite: batchOf(rewrite), semanticReview, maxSemanticRounds: 2 });
  ...
  await check("semantic unresolved after maxSemanticRounds is reported (never silently passed)", async () => {
    const segs = [{ lines: [{ lineIndex: 0, speakerName: "Zabala", isFactualClaim: true, text: "The Yankees are 39-48.", evidenceRefs: [{ type: "game", id: "f1" }] }] }];
    ...
    const r = await selfVerifyAndCorrect(segs, { evidenceByRefId: evByRef, fullEvidenceText: "The Orioles are 39-48.", hostNames: ["Zabala", "Mercer"], rewrite: batchOf(rewrite), semanticReview, maxSemanticRounds: 2 });
  ...
  await check("FIX 3: a rewrite preserves the trailing em-dash of an interruption predecessor", async () => {
    ...
    const r = await selfVerifyAndCorrect(segs, { evidenceByRefId: evByRef, fullEvidenceText: "Detroit hit three home runs.", hostNames: ["Zabala", "Mercer"], rewrite: batchOf(rewrite), maxAttempts: 2 });
