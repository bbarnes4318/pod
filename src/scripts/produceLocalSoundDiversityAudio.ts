...
async function main() {
  ...
  try {
    ...
    const renderEpisode = async (spec: SeriesSpec, i: number, podKey: string, ownerId: string | null, podcastId: string | null) => {
      ...
      const script = await db.script.create({ data: { episodeId: ep.id, version: 1, status: "approved", plainText: lineTexts.join(" "), content: { segments: [{ type: "topic", lines: [mkLine(0), mkLine(1)] }, { type: "topic", lines: [mkLine(2), mkLine(3)] }] } as object } });
