...
export function Breadcrumbs({ items }: { items: Array<{ label: string; href?: string }> }) {
  return <nav className={styles.breadcrumbs} aria-label="Breadcrumb">{items.map((item, index) => <span key={`${item.label}-${index}`}>{index > 0 && <span aria-hidden="true">/ </span>}{item.href ? <Link href={item.href}>{item.label}</Link> : <span aria-current="page">{item.label}</span>}</span>)}</nav>;
}

export function PageHeader({ title, description, actions, detail = false, breadcrumbs }: { title: string; description?: string; actions?: ReactNode; detail?: boolean; breadcrumbs?: Array<{ label: string; href?: string }> }) {
  return <header className={styles.pageHeader}><div className={styles.pageHeaderCopy}>{breadcrumbs && <Breadcrumbs items={breadcrumbs} />}<h1 className={`${styles.title} ${detail ? styles.detailTitle : ""}`}>{title}</h1>{description && <p className={styles.description}>{description}</p>}</div>{actions && <PageActions>{actions}</PageActions>}</header>;
}
...
export function Pagination({ page, pageSize, total, basePath, query = {} }: { page: number; pageSize: number; total: number; basePath: string; query?: Record<string, string | undefined> }) {
  ...
  const href = (next: number) => { const params = new URLSearchParams(); Object.entries(query).forEach(([key, value]) => { if (value) params.set(key, value); }); params.set("page", String(next)); return `${basePath}?${params.toString()}`; };
  const start = total === 0 ? 0 : (page - 1) * pageSize + 1;
  const end = Math.min(total, page * pageSize);
  return <nav className={styles.pagination} aria-label="Pagination"><span>{start}–{end} of {total}</span><div className={styles.paginationActions}>{page > 1 ? <ButtonLink href={href(page - 1)} size="compact">Previous</ButtonLink> : <Button size="compact" disabled>Previous</Button>}<span>Page {page} of {pages}</span>{page < pages ? <ButtonLink href={href(page + 1)} size="compact">Next</ButtonLink> : <Button size="compact" disabled>Next</Button>}</div></nav>;
}
...
export function Tabs({ items, current }: { items: Array<{ label: string; href: string; value: string }>; current: string }) { return <><nav className={styles.tabs} aria-label="Sections">{items.map(item => <Link key={item.value} href={item.href} className={styles.tab} aria-current={item.value === current ? "page" : undefined}>{item.label}</Link>)}</nav><Select className={styles.responsiveSelect} value={current} aria-label="Section" readOnly>{items.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}</Select></>; }
